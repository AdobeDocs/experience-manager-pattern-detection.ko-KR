...
>title="워크플로"
>abstract="WRK 코드는 워크플로 모델 또는 시작 관리자 관련 검색 결과를 식별합니다. 이러한 식별은 AEM as a Cloud Service으로 업그레이드할 때 사용자 지정 에셋 워크플로 모델을 마이그레이션해야 하기 때문에 보고됩니다. AEM as a Cloud Service을 사용하면 에셋 마이크로서비스에서 에셋 처리를 수행합니다."
...
>title="구현 지침"
>abstract="자산 마이크로서비스는 표준 자산 워크플로를 자동으로 지원합니다. 따라서 가장 좋은 방법은 모든 사용자 지정 에셋 워크플로 모델 또는 런처를 검토하는 것입니다. 검토 시 AEM as a Cloud Service으로 전환한 후 필요 여부를 확인할 수 있습니다. 에셋 워크플로를 맞춤화하려면 에셋 워크플로 마이그레이션 도구의 도움을 받아 AEM에서 as a Cloud Service으로 작동하도록 마이그레이션해야 합니다"
